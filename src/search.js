...
(function() {
	...
	this.configure = function() {
		...
		for (var i_tables = 0; i_tables < _tables._raw_objects.length; i_tables++) {
			...
			console.log(temp_pagination_cache);
			if (temp_pagination_cache.length < 1) {
				...
				for (var i_pagination = 0; i_pagination < temp_pagination_cache.length; i_pagination++) {
					...
					console.log(this_pagination);
					console.log(this_table);
					if (this_pagination.prop("id") == "") {
						this_pagination.prop("id", temp_table_id+"p"+i_pagination);
					}
	...
	this.configurePages = function(input_table_root, input_pagination_id,input_pages_total, input_pages_current) {
		...
		if (temp_pagination_output.length < 1) {
			temp_pagination_output_status = false;
	...
	this.incrementID = function(input_increment_id) {
		if ($("#"+input_increment_id).length !== 0) {
			var temp_input_numbers = input_increment_id.match(/\d+/g).map(Number);
(temp_input_numbers);
			var temp_increment_id = input_increment_id.split(temp_input_numbers)[0]+((Number(temp_input_numbers))++);
			//var temp_increment_id = input_increment_id+($("#"+input_increment_id).length);
			console.log(input_increment_id);
			console.log(temp_increment_id);
